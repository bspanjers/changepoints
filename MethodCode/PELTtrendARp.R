...
PELT.trendARp=function(data,p=1, q=q, pen=0,minseglen=1,verbose=FALSE){
  ...
  trendARpsegfit=function(data,start,p){
    ...
    #X=cbind(1, start:(start+n-1))  # column of ones + time index
    #arfit <- arima(data,order = c(p, 0, 1),xreg = X,include.mean = FALSE,method = "ML")
    #print(arfit$coef)
    #print(trendfit$coef)
...
fit.trendARp = function(data, cpts, p=1, q=0, dates=NULL, plot=T, fit=T, add.ar=F, title="Data") {
  ...
  trendARpsegfit = function(data, start, p) {
    ...
    X = start:(start + n - 1)
    trendfit = lm(data ~ X)
    arfit = arima(resid(trendfit), order=c(p,0,q), include.mean=FALSE, method="CSS")
    return(list(
