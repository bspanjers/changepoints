...
run_bootstrap_for_setting <- function(setting_id, nsim, refyear, lag_thresh, penalty, tau_star, years, n) {
  ...
  if (length(valid_results) > 0) {
    ...
    # Build year-wise rejection frequencies for valid 1-break cases
    years_detected <- as.character(sapply(valid_results, function(x) x$year))
    valid_1break <- table(years_detected)
    reject_1break <- table(years_detected[rejections])

    all_years <- sort(unique(as.integer(names(valid_1break))))
    rej_year_df <- data.frame(
      Year = all_years,
      RejectPct = sapply(all_years, function(yr) {
        yr_chr <- as.character(yr)
        n_valid_yr <- as.numeric(valid_1break[[yr_chr]])
        n_rej_yr <- if (yr_chr %in% names(reject_1break)) as.numeric(reject_1break[[yr_chr]]) else 0
        if (is.na(n_valid_yr) || n_valid_yr == 0) return(NA_real_)
        n_rej_yr / n_valid_yr
      }),
      check.names = FALSE
    )
    ...
    rejection_rate <- NA
    rej_year_df <- data.frame(Year = numeric(0), RejectPct = numeric(0))
    year_table <- NULL
  }

  table1_stats <- data.frame(
    Statistic = c("No break", "Two or more breaks", "Break far from 1973"),
    Value = c(n_zero / nsim, n_2ormore / nsim, n_far / nsim),
    stringsAsFactors = FALSE
  )

  # Pretty display table with 3 side-by-side Year/Reject % pairs
  format_table1_display <- function(rej_df, n_blocks = 3) {
    if (nrow(rej_df) == 0) return(data.frame())

    rows_per_block <- ceiling(nrow(rej_df) / n_blocks)
    blocks <- vector("list", n_blocks)

    for (b in seq_len(n_blocks)) {
      idx_start <- (b - 1) * rows_per_block + 1
      idx_end <- min(b * rows_per_block, nrow(rej_df))

      if (idx_start > nrow(rej_df)) {
        block <- rej_df[0, , drop = FALSE]
      } else {
        block <- rej_df[idx_start:idx_end, , drop = FALSE]
      }
      names(block) <- c(
        paste0("Year", ifelse(b == 1, "", paste0("_", b))),
        paste0("RejectPct", ifelse(b == 1, "", paste0("_", b)))
      )
      blocks[[b]] <- block
    }

    max_rows <- max(sapply(blocks, nrow))
    padded <- lapply(blocks, function(df) {
      if (nrow(df) < max_rows) {
        pad <- as.data.frame(matrix(NA, nrow = max_rows - nrow(df), ncol = ncol(df)))
        names(pad) <- names(df)
        df <- rbind(df, pad)
      }
      df
  ...
  table1 <- list(
    statistics = table1_stats,
    rejection_by_year = rej_year_df,
    rejection_by_year_display = format_table1_display(rej_year_df)
  )
